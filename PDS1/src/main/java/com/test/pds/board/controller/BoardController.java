...
public class BoardController {
	...
	public String getBoardList(Model model
		...
		Map<String,Object> map =boardService.getBoardList(currentPage,pagePerRow);//페이징처리 currentPage 현재페이지 , pagePerRow 해당페이지에 보여줄 게시물 갯수
	...
	public String getDetailList(Model model
		...
		logger.debug("boardController.getDetailList : "+boardId);//게시물 클릭시  id 를 받아와 그에맞는 list 를  jsp 에 보여준다.
	...
	public String modifyBoard(Model model
							,@RequestParam(value="boardId")int boardId) {
		logger.debug("boardController.modifyBoard");
		List<Board> list =boardService.getDetailList(boardId);//id 를 받아와 수정 페이지를 나타내준다
	...
	public String modifyBoard(Model model
		...
		logger.debug("boardController.modifyBoard");//수정 form 에 정보입력시 해당 정보를 처리한다.
		boardService.modifyBoard(boardRequest);
		model.addAttribute("boardId", boardRequest.getBoardId());
		return "redirect:/getBoardContent";
	}
